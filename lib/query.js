...
async function dayBlock(dayStart, offsetMs) {
  ...
  if (resting === null) {
    // One aggregate over the whole 20:00–08:00 window — a bucketed series here
    // could split the window across two buckets and quietly return only the
    // evening's minimum.
    const night = await db.aggregate('heart-rate', dayStart - 4 * 3600000, dayStart + 8 * 3600000, 'min');
    if (night !== null) { resting = night; restingSource = 'derived'; }
  }
...
async function assistantDigest(nowMs = Date.now(), offsetMs = TZ_OFFSET_MS) {
  ...
  // The last 30 civil days, newest first. Thirty are fetched because the 30-day
  // averages below need them; only the first 14 ship as `recentDays`, which keeps
  // the prompt-visible list readable.
  const days = [];
  for (let i = 0; i < 30; i++) {
