...
async function assistantDigest(nowMs = Date.now(), offsetMs = TZ_OFFSET_MS) {
  ...
  const missing = catalog.all().filter((t) => t.primary && !withData.some((s) => s.data_type === t.id));
  if (missing.length) {
    notes.push(`No data at all for: ${missing.map((t) => t.label).join(', ')} — the device does not report these.`);
  }
  notes.push('Cardio load and HR zones are DERIVED from raw heart-rate samples, not reported by Google Health.');
