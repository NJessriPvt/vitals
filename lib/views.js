...
async function dayPayload(dateStr, offsetMs) {
  ...
  const { age, maxHeartRate: maxHr, maxHeartRateSource: maxHrSource } = profile;
  ...
  const [
    ...
    metrics.zoneBreakdown(from, effectiveTo, { bucketMs: 3600000, offsetMs, maxHr, maxHrSource }),
...
async function overviewPayload(days, offsetMs) {
  ...
  const [steps, distance, activeCal, totalCal, azm, sleep, hr, zones] = await Promise.all([
    ...
    metrics.zoneBreakdown(from, to, { bucketMs: DAY_MS, offsetMs, maxHr, maxHrSource }),
  ]);
  ...
  const sum = (key, p = 0) => {
    const vals = rows.map((r) => r[key]).filter((v) => v !== null && Number.isFinite(v));
    return vals.length ? round(vals.reduce((a, b) => a + b, 0), p) : null;
  };
  ...
  return {
    ...
    totals: {
      steps: sum('steps'),
      // Same null semantics as the siblings: an empty store means "not measured",
      // never a measured 0 km.
      distanceKm: sum('distanceKm', 1),
...
module.exports = {
  dayPayload, overviewPayload, sleepPayload, sleepForDay, dayRange, todayString, DAY_MS,
};
