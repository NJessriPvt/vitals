...
async function tailType(getToken, typeId) {
  ...
  // setCursor is atomic (LEAST/GREATEST in SQL), so no JS-side min against a
  // snapshot — a concurrent backfill write cannot be lost.
...
/**
 * Read the account holder's age from Google and store it for the derived layer.
 *
 * Age is an input to every heart-rate zone boundary (`220 - age`), so it belongs with
 * the rest of the synced state rather than in a form the user has to remember to
 * update. It rides the tail sweep so it inherits the lease — one replica, on cadence.
 *
 * It must NEVER fail the sweep. A profile read that errors leaves the last known age
 * in place, and a stale-by-one-sweep age is a far better zone boundary than a sweep
 * that stopped before fetching any heart rate.
 */
async function syncUserProfile(getToken) {
  try {
    const profile = await health.userProfile(getToken);
    const age = Number(profile && profile.age);
    // The API returns whole years, so there is no birth date to age forward and
    // nothing to recompute between syncs — an absent or nonsensical figure simply
    // leaves the stored value alone.
    if (!Number.isFinite(age)) return null;
    const stored = await metrics.setAge(age, 'google');
    return stored;
  } catch (err) {
    if (err.code === 'not_connected') throw err;
    log('warn', null, `profile age unavailable: ${err.message}`);
    return null;
  }
}
...
async function tailAll(getToken) {
  ...
  for (const id of ids) {
    try {
      ...
      // A refused refresh token fails identically for every remaining type: 33 more
      // requests Google will reject, and ~66 error events per sweep — enough to trim
      // the one event that says what actually went wrong out of the 500-row log.
      // Nothing but a new grant fixes it, so stop the sweep here.
      if (err.code === 'invalid_grant') {
        log('warn', null, 'refresh token refused — abandoning this sweep; reconnect to resume');
        break;
      }
    }
...
async function syncNow(getToken, typeId = null) {
  ...
  try {
    ...
    state.denied.clear();
    // A whole-account manual sync refreshes the age too; a single-type request is a
    // targeted fetch and has no business making an extra upstream call.
    if (!typeId) await syncUserProfile(getToken);
    let fresh = 0;
    for (const id of ids) {
      try {
        const r = await tailType(getToken, id);
...
module.exports = {
  bus, start, stop, syncNow, syncRange, syncUserProfile,
