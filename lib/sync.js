...
function start(getToken) {
  ...
  const tick = async () => {
    ...
    loopRunning = true;
    try {
      const due = !state.lastTailMs || (Date.now() - state.lastTailMs) >= TAIL_INTERVAL_MS;
      if (due) await tailAll(getToken);
      else await backfillStep(getToken);
...
async function status() {
  const cursors = await db.allCursors();
  return {
