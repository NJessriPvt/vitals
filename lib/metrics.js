...
/**
 * Update the physiological inputs used by every derived insight.
 *
 * Validate the whole patch before writing either field so a bad max HR cannot leave
 * a valid age half-applied. `null`/empty max HR deliberately clears the override and
 * returns to the age estimate.
 */
async function setProfile(patch) {
  const next = {};
  if (Object.hasOwn(patch, 'age')) {
  ...
  if (Object.hasOwn(next, 'age')) await db.setSetting('age', next.age);
...
module.exports = {
  ...
  getAge, setAge, getProfile, setProfile,
