...
// Matches the synthetic profile the generated vitals are drawn around.
const DEMO_AGE = 30;
...
// LOCAL civil date, not the UTC slice: dayStart is a local midnight, and on a
// UTC+4 host the UTC slice of that instant is YESTERDAY — which quietly dated
// every daily summary one day early, so "today" had no resting HR, HRV or
// temperature and readiness sat in "calibrating" against a full demo body.
const day = (ms) => {
  const d = new Date(ms);
...
function generate(days = 180, seed = 20260815) {
  ...
  for (let d = days; d >= 0; d--) {
    ...
    // Leave enough ID space for every intraday sample. Heart rate alone can carry
    // 1,440 minute offsets; a 1,000-wide day block makes adjacent days overlap, so
    // MySQL's correct upsert behavior quietly replaces part of yesterday with today.
    const seq = (days - d) * 10000;
    ...
    // Five-minute background cadence keeps the demo small. The workout window uses
    // one-minute samples so the detector and db.heartRateZones() both have honest
    // coverage; both cap one sparse reading at one minute rather than inventing the
    // unobserved part of a five-minute gap.
    for (let m = 0; m < 1440;) {
      const hour = m / 60;
      const asleep = hour < 7 || hour >= 23;
      const workout = active && !asleep && hour > 17.5 && hour < 18.6;
      let hr = asleep
        ? restHr - 4 + rand() * 5
        : restHr + 8 + Math.sin((hour - 6) / 18 * Math.PI) * 12 + rand() * 9;
      // A workout leaves a visible spike, which is what makes the chart readable.
      if (workout) hr += 45 + rand() * 20;
    ...
    // `type`/`stages`, the LIVE field names (the doc example says sleepType/
    // sleepStages) — demo mode must exercise the same branch real data takes.
    push('sleep', envelope('sleep', {
      startTime: iso(bedtime), endTime: iso(wake), type: 'STAGES', stages,
    }, seq));
    ...
    if (d % 3 === 0) {
      push('weight', envelope('weight', {
        ...
        // GRAMS, like the live API (see the measured value-field table): the
        // display layer scales 1/1000, so kilograms here rendered as 0.08 kg.
    ...
    if (d === days) {
      push('height', envelope('height', {
        // Millimetres, like the live API — the display layer scales 1/10 to cm.
        sampleTime: { physicalTime: iso(dayStart) }, heightMillimeters: 1780,
      }, seq));
    }
...
async function load(days = 180) {
  ...
  // The real app reads age from the Google account; the demo has no account, so it
  // seeds the same setting under its own source rather than leaving every zone
  // boundary on the pre-sync fallback and calling that "from Google".
  await metrics.setAge(DEMO_AGE, 'demo');
