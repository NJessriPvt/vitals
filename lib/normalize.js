'use strict';
/**
 * dataPoint (Google's JSON) -> row (our stored shape).
 *
...
function toMs(v) {
  ...
  const s = String(v);
  // Bare civil date (DAILY types). Anchored at UTC midnight of that date, NOT local
  // midnight: the anchor is part of the point's identity, and `new Date(y, m, d)` is
  // process-local — so the same day read by a UTC container and a UTC+4 laptop
  // produced two different anchors, two different ids, and a daily total that
  // double-counted. Local-day placement is applied at QUERY time by the viewer's
  // offset, which is where a timezone belongs.
...
function extractSleep(body, times) {
  ...
  const fields = {
    // The live API sends `type` — `sleepType` is the doc example's spelling
    // (see the measured-facts table in CLAUDE.md). Accept both, live shape first.
    sleepType: body.type || body.sleepType || null,
    inBedMs,
    awakeMs,
...
function civilToMs(c) {
  ...
  // Date.UTC, not `new Date(...)` — see the note in toMs(). A rollup point's id is
  // derived from this value, so a process-local reading makes the same day a
  // different point on a differently-zoned host.
  return Date.UTC(
