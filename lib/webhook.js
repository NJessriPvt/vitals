...
async function loadKeyset(force = false) {
  ...
  for (const entry of json.key || []) {
    ...
    try {
      keys.set(Number(entry.keyId), {
        key: keyFromTinkEntry(entry),
        outputPrefixType: entry.outputPrefixType || 'TINK',
      });
...
module.exports = { handle, verifySignature, loadKeyset, resolveType, secret, SIGNATURE_HEADER };
