...
/** Types the webhook subscriber can push in real time — the API supports only these. */
const WEBHOOK_TYPES = ['steps', 'altitude', 'distance', 'floors', 'weight', 'sleep'];
...
function timeRangeFilter(t, fromMs, toMs) {
  ...
  return `${field} >= "${fmt(fromMs)}" AND ${field} < "${fmt(toMs)}"`;
}

module.exports = {
  INTERVAL, SAMPLE, DAILY, SESSION,
  civilDate, civilDateTime,
  SCOPES, SCOPE_BASE, WEBHOOK_TYPES,
