...
async function listDataPoints(getToken, typeId, fromMs, toMs) {
...
/**
 * The account holder's own profile — age, membership start, configured stride lengths.
 *
 * Measured against the live API: this is a plain GET under the ALREADY-GRANTED
 * `googlehealth.profile.readonly` scope, and `age` arrives as a whole number of years
 * rather than a birth date, so there is nothing to recompute and nothing dated to keep
 * fresh beyond re-reading it. `/users/me` and `/users/me/userProfile` are both 404 —
 * the resource is only reachable at this exact path.
 */
async function userProfile(getToken) {
  return request(getToken, '/users/me/profile');
}
