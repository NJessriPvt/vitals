...
async function verdictsFor(offsetMs) {
  ...
  for (const m of VERDICT_METRICS) {
    ...
    if (!type) continue;
    const recent = await dailySeries(m.id, today - 90 * DAY_MS, today + DAY_MS, offsetMs);
    const prior = await dailySeries(m.id, today - 365 * DAY_MS, today - 90 * DAY_MS, offsetMs);
    out.push({
      ...buildVerdict({
        id: m.id,
        label: m.label,
...
function buildReport(current, priors, extras = {}) {
  ...
  // Strain-vs-capacity balance: the week's load against what fitness could absorb.
  let balance = null;
  if (Number.isFinite(current.loadTotal) && Number.isFinite(extras.fitness) && extras.fitness >= 5) {
    const ratio = current.loadTotal / (extras.fitness * 7);
    balance = {
      ratio: round(ratio, 2),
...
async function weeklyReportFor(dayStart, offsetMs, { loadByDay, fitness } = {}) {
  ...
  const current = await periodAggregates(weekStart, Math.min(weekStart + 7 * DAY_MS, dayStart + DAY_MS), offsetMs, loadByDay);
  const priors = [];
  for (let i = 1; i <= 3; i++) {
    priors.push(await periodAggregates(weekStart - i * 7 * DAY_MS, weekStart - (i - 1) * 7 * DAY_MS, offsetMs, loadByDay));
  }
  return {
    weekStart,
    ...buildReport(current, priors, { fitness }),
    derived: true,
    method: 'This week against the mean of the prior three; balance = week TRIMP ÷ (7×Fitness) — under 0.8 restoring, to 1.25 optimal, above it overreaching.',
  };
}
...
module.exports = {
  DAY_MS, VERDICT_METRICS, CORR_MIN_N, CORR_MIN_R,
