...
async function trainPayload(offsetMs) {
  ...
  const [strengthRows, stepsYear, caloriesSeries] = await Promise.all([
    db.strengthList(now - 60 * DAY_MS, now),
    trends.dailySeries('steps', today - 365 * DAY_MS, today + DAY_MS, offsetMs),
    trends.bandSeriesFor('total-calories', 35, offsetMs),
  ]);
  ...
  return {
    ...
    calories: {
      ...caloriesSeries,
      days: 35,
      method: "The device's own daily total burn — activity plus resting metabolism — "
        + 'from the daily rollup, one bar per day.'
        + (caloriesSeries.band
          ? ` Your typical day is the median of the ${caloriesSeries.band.window === 'prior' ? 'prior' : 'visible'} window.`
          : ''),
      limitation: "Today's bar is still accumulating, and a late watch sync can restate a whole day.",
    },
    records,
    strength: {
