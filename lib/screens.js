...
/** Daily values with the DAILY→sample fallback, as an ordered array of numbers. */
async function dailyWithFallback(primary, fallback, fromMs, toMs, agg, offsetMs) {
  const [p, f] = await Promise.all([
    db.series(primary, fromMs, toMs, DAY_MS, agg, offsetMs),
    fallback ? db.series(fallback, fromMs, toMs, DAY_MS, agg, offsetMs) : [],
  ]);
  const byDay = new Map(f.map((r) => [r.t, r.v]));
  for (const r of p) byDay.set(r.t, r.v);
...
async function readinessInputs(dayStart, offsetMs, loadSeries, sleepRow, need) {
  ...
  return {
    ...
    sleep: { hours: sleepRow ? sleepRow.mainHours : null, needHours: need.baseNeedHours },
...
async function radarFor(dayStart, offsetMs, inputs) {
  ...
  const respBase = inputs.respiratoryRate;
  return scores.buildSymptomRadar([
    ...
    },
    {
      ...
      current: respBase.current,
      band: stats.personalBand(await dailyWithFallback('daily-respiratory-rate', 'respiratory-rate-sleep-summary', baseFrom, dayStart, 'avg', offsetMs)),
      direction: 'high-bad',
    },
    {
      id: 'resting-heart-rate', label: 'Resting HR', unit: 'bpm', precision: 0,
      current: inputs.restingHeartRate.current,
      band: stats.personalBand(await dailyWithFallback('daily-resting-heart-rate', 'heart-rate', baseFrom, dayStart, 'min', offsetMs)),
      direction: 'high-bad',
    },
    {
      id: 'hrv', label: 'HRV', unit: 'ms', precision: 0,
      current: inputs.hrv.current,
      band: stats.personalBand(await dailyWithFallback('daily-heart-rate-variability', 'heart-rate-variability', baseFrom, dayStart, 'avg', offsetMs)),
      direction: 'low-bad',
    },
    {
...
async function todayPayload(dateStr, offsetMs) {
  ...
  const [loadSeries, nightRows, zones, hrHourly, sessionBundle] = await Promise.all([
    training.dailyLoadSeries(to, 35, offsetMs, profile),
    night.dayRows(from, to, offsetMs),
    metrics.zoneBreakdown(from, effectiveTo, { bucketMs: HOUR_MS, offsetMs, maxHr: profile.maxHeartRate }),
  ...
  const need = await night.sleepNeedFor(from, {
  ...
  const sleepRatio = nightRow && nightRow.mainHours !== null && need.baseNeedHours
    ? nightRow.mainHours / need.baseNeedHours : null;
  ...
  const rings = goals.buildRings(adaptive, {
    ...
    sleepHours: nightRow ? nightRow.mainHours : null,
  });
  ...
  return {
    ...
    readiness,
    strain: {
      today: todayLoad && todayLoad.load !== null ? training.strainOf(todayLoad.load) : null,
      load: todayLoad ? todayLoad.load : null,
      target,
      source: todayLoad ? todayLoad.source : null,
      max: 21,
      derived: true,
    ...
    sleepStrip: nightRow ? {
      ...
      naps: nightRow.naps,
    } : { mainHours: null, debtHours: need.debtHours },
    radar,
...
async function trainPayload(offsetMs) {
  ...
  const need = await night.sleepNeedFor(today);
  ...
  return {
    ...
    season: {
      ...
      thresholds: [0.2, 0.4, 0.6, 0.8]
        .map((q) => stats.quantile(loadSeries.map((d) => d.load).filter((v) => v), q))
        .map((v) => round(v, 0)),
    },
    weeklyLoad: weeks,
    records,
...
async function compareSide(dateStr, metric, offsetMs, { hourly = false } = {}) {
  ...
  const [zones, sleepRows] = await Promise.all([
    metrics.zoneBreakdown(from, effTo, { bucketMs: 0, offsetMs, maxHr: profile.maxHeartRate }),
    night.dayRows(from, to, offsetMs),
  ]);
  ...
  if (metric === 'steps' || hourly) {
    ...
    curve = (await db.heartRateTrace(from, effTo, 360)).points;
  }
  return {
    date: dateStr,
    curve,
    metrics: {
      ...
      activeZoneMinutes: round(azm, 0),
      activeCalories: round(activeCal, 0),
      sleepHours: sleepRows[0] ? sleepRows[0].mainHours : null,
...
async function youPayload(offsetMs) {
  ...
  const need = await night.sleepNeedFor(today);
  const nights14 = await night.nightHours(today - 14 * DAY_MS, today + DAY_MS);
...
module.exports = {
  ...
  calendarPayload, readinessInputs, restingFor, hrvFor, dailyWithFallback,
};
